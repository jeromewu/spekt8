const fetchData = (nodes, edges, graph, that) => {
    ...
    const p1 = fetch(`${window.location.origin}/pod`);
    const p2 = fetch(`${window.location.origin}/service`);
    const p3 = fetch(`${window.location.origin}/ingress`);
    const p4 = fetch(`${window.location.origin}/daemonset`);
